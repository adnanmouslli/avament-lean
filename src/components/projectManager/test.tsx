...
//   isDragging: boolean;
//   task: Task | null;
//   type: 'move' | 'resize-left' | 'resize-right' | 'move-multiple';
//   startMouseX: number;
//   startMouseY: number;
//   originalTask: Task | null;
//   nodeId?: string;
//   originalSelected?: Map<string, { task: Task; nodeId: string }>;
//   draggedTasks?: Map<string, Task>;
// }
...
//         // رسم زر التحديد المتعدد
//         drawMultiSelectButton(ctx, rect);
...
//   // فحص النقر على زر التحديد المتعدد
//   if (handleMultiSelectButtonClick(mouseX, mouseY)) {
//     return;
//   }
...
//   handleMultiSelectButtonClick,  // إضافة الدالة الجديدة
//   viewState,
//   scaledDayWidth,
//   scaledTaskHeight,
//   CANVAS_CONFIG,
...
// // ★ إضافة حالة وضع التحديد المتعدد ★
...
// // دالة رسم زر التحديد المتعدد
// const drawMultiSelectButton = useCallback((ctx: CanvasRenderingContext2D, rect: DOMRect) => {
//   const buttonWidth = 100;
//   const buttonHeight = 30;
//   const buttonX = rect.width - buttonWidth - 20; // على اليمين
//   const buttonY = (CANVAS_CONFIG.headerHeight - buttonHeight) / 2;
  
//   // خلفية الزر
//   ctx.fillStyle = multiSelectMode ? '#3b82f6' : '#6b7280';
//   ctx.beginPath();
...
//   ctx.textAlign = 'center';
//   ctx.textBaseline = 'middle';
//   ctx.fillText('Multi Select', buttonX + buttonWidth / 2, buttonY + buttonHeight / 2);
// }, [CANVAS_CONFIG.headerHeight, multiSelectMode]);

// // دالة معالجة النقر على زر التحديد المتعدد
// const handleMultiSelectButtonClick = useCallback((mouseX: number, mouseY: number) => {
//   const buttonWidth = 100;
//   const buttonHeight = 30;
//   const buttonX = canvasRef.current?.width! / window.devicePixelRatio - buttonWidth - 20; // حساب العرض الفعلي
//   const buttonY = (CANVAS_CONFIG.headerHeight - buttonHeight) / 2;
...
// // في drawCanvas، أضف رسم مربع التحديد إذا كان موجوداً
// // داخل drawCanvas، بعد رسم المهام:
...
// // في handleCanvasMouseDown، بعد التحقق من المهام:
// if (multiSelectMode && !clickedTask) {
//   // بدء مربع التحديد إذا كان الوضع مفعل ولم ينقر على مهمة
//   setSelectionStart({ x: mouseX, y: mouseY });
//   setSelectionEnd({ x: mouseX, y: mouseY });
//   return;
// }

// // في handleCanvasMouseMove:
// if (multiSelectMode && selectionStart) {
//   setSelectionEnd({ x: mouseX, y: mouseY });
// }

// // في handleCanvasMouseUp:
// if (multiSelectMode && selectionStart && selectionEnd) {
...
//   flattened.forEach(node => {
//     if (!node.isLeaf || !node.tasks || !node.yPosition) return;
...
//       let taskWidth, taskHeight;
//       if (task.type === 'milestone') {
//         const size = CANVAS_CONFIG.milestoneSize * 1.5 * viewState.zoom;
//         taskWidth = size;
//         taskHeight = size;
//         taskX -= size / 2; // مركز المعلم
//         taskY += scaledTaskHeight / 2 - size / 2;
//       } else {
//         taskWidth = displayDuration * scaledDayWidth;
//         taskHeight = scaledTaskHeight;
//       }
      
//       // التحقق من التقاطع
//       if (
//         taskX < maxX && taskX + taskWidth > minX &&
//         taskY < maxY && taskY + taskHeight > minY
//       ) {
//         newSelected.set(task.id, { task, nodeId: node.id });
//       }
//     });
//   });
...
// // لتحريك المهام المتعددة، في handleCanvasMouseDown:
// if (clickedTask && clickedNode && selectedTasks.has(clickedTask.id) && selectedTasks.size > 1) {
//   // سحب متعدد إذا كانت المهمة جزء من التحديد
//   setTaskDragState({
//     isDragging: true,
//     task: null, // لا مهمة واحدة
//     type: 'move-multiple',
//     startMouseX: mouseX,
//     startMouseY: mouseY,
//     originalTask: null,
//     originalSelected: new Map(selectedTasks), // حفظ الأصلي
//     draggedTasks: new Map(Array.from(selectedTasks).map(([id, {task, nodeId}]) => [id, {...task}])),
//     nodeId: undefined // قد تكون متعدد nodes
//   });
//   canvas.style.cursor = 'grabbing';
//   return;
// }

// // في handleCanvasMouseMove، إذا type === 'move-multiple':
// if (taskDragState.type === 'move-multiple' && taskDragState.draggedTasks && taskDragState.originalSelected) {
...
//   const daysDelta = Math.round((deltaX / scaledDayWidth) * scaleFactor);
//   const rowSpacing = CANVAS_CONFIG.rowHeight * viewState.zoom;
//   const rowsDelta = Math.round(deltaY / rowSpacing);
  
//   const newDragged = new Map(taskDragState.draggedTasks);
  
//   taskDragState.originalSelected.forEach(({task: originalTask, nodeId}) => {
//     const newTask = { ...originalTask };
//     newTask.startDay += daysDelta;
//     newTask.row = (newTask.row || 0) + rowsDelta;
//     newTask.row = Math.max(0, newTask.row);
    
//     // حدود المشروع مؤقتاً (سيتم تصحيح في mouseUp)
//     newTask.startDay = Math.max(0, newTask.startDay);
    
//     newDragged.set(originalTask.id, newTask);
//   });
  
//   setTaskDragState(prev => ({ ...prev, draggedTasks: newDragged }));
// }

// // في handleCanvasMouseUp، إذا type === 'move-multiple':
// if (taskDragState.type === 'move-multiple' && taskDragState.draggedTasks) {
//   const totalDays = getTotalProjectDays();
...
//           const dragged = taskDragState.draggedTasks!.get(task.id);
//           if (!dragged) return task;
          
//           let finalTask = { ...dragged };
...
//           // حدود المشروع
//           finalTask.startDay = Math.max(0, Math.min(totalDays - 1, finalTask.startDay));
...
//   setSelectedTasks(new Map()); // إلغاء التحديد بعد التحريك
// }

// // في drawCanvas، عند رسم task:
// let displayTask = task;

// // إذا كان سحب متعدد، استخدم النسخة المسحوبة إذا موجودة
// if (taskDragState.type === 'move-multiple' && taskDragState.draggedTasks?.has(task.id)) {
//   displayTask = taskDragState.draggedTasks.get(task.id)!;
// } else if (taskDragState.isDragging && taskDragState.task?.id === task.id && taskDragState.nodeId === node.id) {
//   displayTask = taskDragState.task;
// }

// // ثم استخدم displayTask في الرسم

// // في handleKeyDown، إذا delete، احذف selected
// // إلخ.